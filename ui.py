...
class SimulationUI:
    """Класс для управления пользовательским интерфейсом симуляции."""
    ...
    def _update_shape_buttons(self):
        """Обновление состояния кнопок выбора формы."""
    ...
    def _update_plot_buttons(self):
        """Обновление состояния кнопок построения графиков."""
        default_button_fg = ctk.ThemeManager.theme["CTkButton"]["fg_color"]
        very_light_fg_color = self._lighten_color(default_button_fg[0], 0.7)
    ...
    def _update_ui_state(self):
        ...
        if self.elasticity_slider.cget("progress_color") != elasticity_colors["progress_color"]:
            self.elasticity_slider.configure(progress_color=elasticity_colors["progress_color"],
                                            fg_color=elasticity_colors["fg_color"])
        ...
        if self.friction_slider.cget("progress_color") != friction_colors["progress_color"]:
            self.friction_slider.configure(progress_color=friction_colors["progress_color"],
                                          fg_color=friction_colors["fg_color"])
        ...
        if self.velocity_slider.cget("progress_color") != velocity_colors["progress_color"]:
            self.velocity_slider.configure(progress_color=velocity_colors["progress_color"],
                                          fg_color=velocity_colors["fg_color"])
        ...
        if self.angle_slider.cget("progress_color") != angle_colors["progress_color"]:
            self.angle_slider.configure(progress_color=angle_colors["progress_color"],
                                       fg_color=angle_colors["fg_color"])
        ...
        if self.air_resistance_slider.cget("progress_color") != air_colors["progress_color"]:
            self.air_resistance_slider.configure(progress_color=air_colors["progress_color"],
                                                fg_color=air_colors["fg_color"])
    ...
    def _complete_ui_update(self):
        """Завершение обновления интерфейса."""
        self._is_switching_mode = False
        self.root.update_idletasks()

    def _safe_set_var(self, var, entry):
        """Безопасная установка значения переменной."""
        if (var in [self.velocity_var, self.angle_var, self.air_resistance_var] and
                config.mode == "slingshot") or \
        ...
        try:
            ...
            elif var == self.elasticity_var:
                value = max(0, min(1, value))
            elif var == self.friction_var:
                value = max(0, min(1, value))
            elif var == self.gravity_var:
                value = max(0, min(50, value))
            elif var == self.velocity_var:
                value = max(0, min(15, value))
            elif var == self.angle_var:
                value = max(0, min(90, value))
            elif var == self.air_resistance_var:
                value = max(0, min(50, value))
            var.set(round(value, precision))
            entry.delete(0, ctk.END)
            entry.insert(0, str(var.get()))
        except ValueError:
            entry.delete(0, ctk.END)
    ...
    def _create_widgets(self):
        ...
        ctk.CTkLabel(self.mass_frame, text="Масса (кг):", width=150, anchor="w",
                     text_color=self._get_theme_color("fg")).pack(side="left", padx=5)
        self.mass_slider = ctk.CTkSlider(self.mass_frame, from_=1, to=100,
        ...
        self.mass_entry.bind("<Return>", lambda e: self._safe_set_var(self.mass_var,
                                                                     self.mass_entry))

        self.radius_frame = ctk.CTkFrame(self.root, fg_color=self._get_theme_color("bg"))
        self.radius_frame.pack(fill="x", padx=10, pady=5)
        ctk.CTkLabel(self.radius_frame, text="Радиус (м):", width=150, anchor="w",
                     text_color=self._get_theme_color("fg")).pack(side="left", padx=5)
        self.radius_slider = ctk.CTkSlider(self.radius_frame, from_=10, to=100,
        ...
        self.radius_entry.bind("<Return>", lambda e: self._safe_set_var(self.radius_var,
                                                                       self.radius_entry))
        ...
        self.elasticity_label = ctk.CTkLabel(self.elasticity_frame, text="Упругость:",
                                             width=150, anchor="w",
                                             text_color=self._get_theme_color("fg"))
        self.elasticity_label.pack(side="left", padx=5)
        self.elasticity_slider = ctk.CTkSlider(self.elasticity_frame, from_=0, to=1,
        ...
        self.elasticity_entry.bind("<Return>", lambda e: self._safe_set_var(self.elasticity_var,
                                                                           self.elasticity_entry))
        ...
        self.friction_label = ctk.CTkLabel(self.friction_frame, text="Трение:", width=150,
                                           anchor="w",
                                           text_color=self._get_theme_color("fg"))
        self.friction_label.pack(side="left", padx=5)
        self.friction_slider = ctk.CTkSlider(self.friction_frame, from_=0, to=1,
        ...
        self.friction_entry.bind("<Return>", lambda e: self._safe_set_var(self.friction_var,
                                                                         self.friction_entry))
        ...
        ctk.CTkLabel(self.gravity_frame, text="Гравитация (м/с²):", width=150, anchor="w",
                     text_color=self._get_theme_color("fg")).pack(side="left", padx=5)
        self.gravity_slider = ctk.CTkSlider(self.gravity_frame, from_=0, to=50,
        ...
        self.gravity_entry.bind("<Return>", lambda e: self._safe_set_var(self.gravity_var,
                                                                        self.gravity_entry))
        ...
        self.velocity_label = ctk.CTkLabel(self.velocity_frame, text="Скорость (м/с):",
                                           width=150, anchor="w",
                                           text_color=self._get_theme_color("fg"))
        ...
        self.velocity_entry.bind("<Return>", lambda e: self._safe_set_var(self.velocity_var,
                                                                         self.velocity_entry))

        self.angle_frame = ctk.CTkFrame(self.root, fg_color=self._get_theme_color("bg"))
        self.angle_frame.pack(fill="x", padx=10, pady=5)
        self.angle_label = ctk.CTkLabel(self.angle_frame, text="Угол наклона (град):",
                                        width=150, anchor="w",
                                        text_color=self._get_theme_color("fg"))
        self.angle_label.pack(side="left", padx=5)
        self.angle_slider = ctk.CTkSlider(self.angle_frame, from_=0, to=90,
        ...
        self.angle_entry.bind("<Return>", lambda e: self._safe_set_var(self.angle_var,
                                                                      self.angle_entry))

        self.air_resistance_frame = ctk.CTkFrame(self.root,
                                                fg_color=self._get_theme_color("bg"))
        self.air_resistance_frame.pack(fill="x", padx=10, pady=5)
        self.air_resistance_label = ctk.CTkLabel(self.air_resistance_frame,
                                                 text="Сопротивление воздуха:",
                                                 width=150, anchor="w",
                                                 text_color=self._get_theme_color("fg"))
        self.air_resistance_label.pack(side="left", padx=5)
        self.air_resistance_slider = ctk.CTkSlider(self.air_resistance_frame, from_=0,
                                                   to=10, variable=self.air_resistance_var,
                                                   command=self.update_air_resistance,
                                                   width=200)
        ...
        self.air_resistance_entry.bind("<Return>", lambda e: self._safe_set_var(
            self.air_resistance_var, self.air_resistance_entry))
        ...
        self.theme_switch = ctk.CTkSwitch(
            self.root,
            text="Тема: Светлая" if config.theme == "light" else "Тема: Тёмная",
        ...
        self.velocity_plot_button = ctk.CTkButton(
            self.plot_button_frame, text="График скорости",
            command=lambda: plot_velocity_vs_time(self.physics.cannon), width=120)
        self.velocity_plot_button.pack(side="left", padx=(0, 5))
        self.range_plot_button = ctk.CTkButton(
            self.plot_button_frame, text="График дальности",
    ...
    def _update_shape_button_commands(self):
        """Обновление команд кнопок выбора формы."""
        if config.mode == "cannon":
    ...
    def toggle_theme(self):
        ...
        for frame in [self.mass_frame, self.radius_frame, self.elasticity_frame,
                      self.friction_frame, self.gravity_frame, self.velocity_frame,
                      self.angle_frame, self.air_resistance_frame]:
    ...
    def update_air_resistance(self, value):
        """Обновление значения сопротивления воздуха."""
        if config.mode == "slingshot":
            self.air_resistance_var.set(self.INITIAL_AIR_RESISTANCE)
            config.air_resistance = self.INITIAL_AIR_RESISTANCE
            self.air_resistance_entry.delete(0, ctk.END)
    ...
    def clear_objects(self):
        """Очистка объектов в физическом мире."""
        self.physics.clear_objects()

    def set_slingshot_mode(self):
        """Установка режима рогатки."""
        if config.mode == "slingshot":
            return
    ...
    def toggle_color_effect(self):
        """Переключение эффекта цвета."""
        config.color_effect = not config.color_effect
        self.color_effect_switch.configure(
            text="Эффект: " + ("Вкл" if config.color_effect else "Выкл"))
