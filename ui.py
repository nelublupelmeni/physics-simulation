...
class SimulationUI:
    ...
    def _update_plot_buttons(self):
        default_fg_color = ctk.ThemeManager.theme["CTkButton"]["fg_color"]
        light_fg_color = self._lighten_color(default_fg_color[0], 0.3)
        fg_color = light_fg_color if config.mode == "slingshot" else default_fg_color
        self.velocity_plot_button.configure(fg_color=fg_color)
        self.range_plot_button.configure(fg_color=fg_color)
    ...
    def _create_widgets(self):
        ...
        ctk.CTkLabel(self.elasticity_frame, text="Упругость:", width=150, anchor="w", 
                     text_color=config.ui_colors[config.theme]["fg"]).pack(side="left", padx=5)
        ...
        ctk.CTkLabel(self.friction_frame, text="Трение:", width=150, anchor="w", 
                     text_color=config.ui_colors[config.theme]["fg"]).pack(side="left", padx=5)
